...
class Student:
    ...
    def __call__(self, subject, grade, test_result):
        if subject not in self.subjects:
            raise ValueError(f"{subject} не допустимое значение.")
        if grade < 2 or grade > 5:
            raise ValueError("Значение оценки должно быть между 2 и 5 .")
        if test_result < 0 or test_result > 100:
            raise ValueError("Значение результата должно быть между 0 и 100 .")
